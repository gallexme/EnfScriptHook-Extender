...
BOOL WINAPI DllMain( HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved ) {
	...
	if ( fdwReason == DLL_PROCESS_ATTACH ) {
		...
		//HANDLE hThread = ::CreateThread(NULL, 0, (::LPTHREAD_START_ROUTINE)&Hook, (::HMODULE)hinstDLL, 0, NULL);
...
void* Hooked(char* messageA,char* messageB) {
	printf("SUP A: %s B: %s",messageA,messageB);
...
int64_t* MyInitModules(int64_t scriptmoduleaddress ){
	...
	printf("SUP");
		...
		AddFunction fAddFunctions = (AddFunction)(moduleBase + (FUNCTION_AddFunction - EXE_BASE));
		fAddFunctions(scriptmoduleaddress, "Hooked", reinterpret_cast<int64_t>(&Hooked),0);
