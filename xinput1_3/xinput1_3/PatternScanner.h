...
namespace hl
{
	...
	public:
		...
		std::vector<uintptr_t> find( std::vector<std::string>& strings,  std::string& moduleName);
		/// Searches for one specific referenced string within the code of a module.
		uintptr_t findString( std::string& str,  std::string& moduleName, int instance = 0);
		...
		std::map<std::string, uintptr_t> findMap(std::vector<std::string>& strings,
	...
	uintptr_t FindPatternMask(const char* byteMask, const char* checkMask, std::string& moduleName,
		int instance = 0);
	...
	uintptr_t FindPattern(std::string& pattern, std::string& moduleName, int instance = 0);
	/// \overload
	uintptr_t FindPattern(std::string& pattern, uintptr_t address, size_t len, int instance = 0);
	...
	/// Returns a vector of hl::MemoryRegion%s.
	 std::vector<hl::MemoryRegion>& GetCodeRegions( std::string& moduleName);
}
